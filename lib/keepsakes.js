...
/**
 * Delete named images from one set: their storage objects first, then their
 * rows, for the same reason purgeSets does it in that order.
 *
 * A set that loses its last image is deleted outright. An empty set would list
 * as a card with nothing in it, and every other part of this feature already
 * treats "nothing to show" as "do not exist".
 *
 * Returns { removedImages, removedObjects, setDeleted, error }.
 */
export async function purgeImages(sb, setId, indices) {
  const idx = [...new Set((indices || []).map(Number).filter(Number.isInteger))];
  if (!idx.length) {
    return { removedImages: 0, removedObjects: 0, setDeleted: false, error: null };
  }

  const { data: images, error: readErr } = await sb
    .from("keepsake_images")
    .select("idx, storage_path, thumb_path")
    .eq("set_id", setId)
    .in("idx", idx);
  if (readErr) return { removedImages: 0, removedObjects: 0, setDeleted: false, error: readErr };
  if (!images?.length) {
    return { removedImages: 0, removedObjects: 0, setDeleted: false, error: null };
  }
  ...
  if (paths.length) {
    const { error: rmErr } = await sb.storage.from(KEEPSAKE_BUCKET).remove(paths);
    // A missing object must not block the row going: the goal is that nothing
    // survives, and a row pointing at nothing is the worse of the two states.
    if (rmErr) console.warn("[keepsakes] storage remove failed:", rmErr.message);
  }
  ...
  const { error: delErr } = await sb
    ...
    .delete()
    .eq("set_id", setId)
    .in("idx", images.map((i) => i.idx));
  if (delErr) {
    return { removedImages: 0, removedObjects: paths.length, setDeleted: false, error: delErr };
  }
  ...
  if (!count) {
    const { error: setErr } = await sb.from("keepsake_sets").delete().eq("id", setId);
    if (setErr) console.warn("[keepsakes] empty set delete failed:", setErr.message);
    else setDeleted = true;
  }

  return { removedImages: images.length, removedObjects: paths.length, setDeleted, error: null };
}
