...
/**
 * Remove the speech_events rows an earlier, shorter pass wrote for this key.
...
export async function clearSupersededEvents(sb, key) {
  if (!sb || !key?.sessionId) return;
  try {
    const { error } = await applyKey(sb.from("speech_events").delete(), key);
    if (error) console.warn("[session-analysis-store] supersede cleanup skipped", error?.message || error);
  } catch (e) {
    console.warn("[session-analysis-store] supersede cleanup skipped", e?.message || e);
  }
}
