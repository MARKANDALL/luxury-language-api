...
//
// Admin authentication for these routes now lives in lib/admin-auth.js and
// compares against ADMIN_KEY_PRIVATE — see the note there for why ADMIN_TOKEN
// is not fit to gate admin-only surfaces. What remains here is the Vercel cron
// check (CRON_SECRET via Authorization: Bearer) plus small transport helpers.
...
export function isVercelCron(req) {
  ...
  return !!bearer && secretsMatch(bearer, secret);
}
