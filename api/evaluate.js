...
export default async function handler(req, res) {
  ...
  try {
    ...
    const referenceText = fields.referenceText || fields.text || ""; // <-- This is crucial
    if (!referenceText) throw new Error("No reference text found in upload fields.");

    // ---- Correct: Use the constructor, not fromJSON ----
    const paConfig = new sdk.PronunciationAssessmentConfig(
      referenceText,
      sdk.PronunciationAssessmentGradingSystem.HundredMark,
      sdk.PronunciationAssessmentGranularity.Phoneme,
      true // enable prosody/miscue
    );
    ...
    // --- 4. Build response ---
    res.status(200).json(extractPronunciationAndProsody(data, referenceText));
  } catch (err) {
...
function extractPronunciationAndProsody(data, referenceText) {
