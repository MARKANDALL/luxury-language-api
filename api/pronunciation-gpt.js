...
// ------------------------------------------------------------
//  Returns JSON { sections:[ … ] }  (6 bilingual coaching blocks)
// ------------------------------------------------------------
...
const langs = { es:"Spanish", fr:"French", pt:"Portuguese", zh:"Chinese",
...
const sections = [
  ...
  { emoji:"🤝", en:"Reassurance",     min:40, max:70 }     // shorter
];
...
function stripFences(txt){
  if(txt.startsWith("```")) txt=txt.replace(/^```[a-z]*\s*/i,"").replace(/```$/,"");
...
async function translateMissing(sectionsArr, langCode){
  ...
  if(!need.length || langCode==="universal"||!langCode) return;
...
JSON array of the same length, no extra keys.
  ...
  let translations=JSON.parse(stripFences(result.choices[0].message.content));
...
"l1" blank only if langCode == "universal".
No markdown, no triple-backticks.`.trim();
    ...
    try{ data=JSON.parse(raw); }
      ...
      try{
        ...
        console.error("💥 still broken after repair:",repair.choices[0].message.content);
        throw new Error("Malformed JSON after repair");
      }
    }
