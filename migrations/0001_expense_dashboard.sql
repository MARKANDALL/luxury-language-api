-- migrations/0001_expense_dashboard.sql
-- Lux expense dashboard — foundation schema + seed (today's truth: 2026-07-04).
--
-- Idempotent: safe to run repeatedly (Supabase SQL editor OR
-- POST /api/admin/expenses/migrate, which runs the identical string exported
-- from lib/expenses/migration.js — keep the two in sync).
--
-- PRIME LAW: a new cost source is an INSERT into expense_sources, never a
-- schema change. period_start / period_end are plain dates so past periods
-- (the later backfill project) drop in with no migration.
